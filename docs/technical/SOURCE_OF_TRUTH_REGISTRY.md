...
| `docs/technical/CLAUDE_CODEX_OPERATING_MODEL.md` | Day-to-day operating split for using Claude and Codex against one shared SDD core. | technical-director | coordination-policy | technical-director with human approval for behavioral changes. | Multi-runtime workflow, handoff, ownership, or escalation model changes. | Accepted ADRs and coordination rules win; this playbook operationalizes them. | Direct review against ADR-007, `coordination-rules.md`, and `codex-compatibility.md`. | Operational |
...
| `.tasks/handoffs/*` | Optional durable handoff artifacts for High-risk cross-domain transfers. | sending agent; producer coordinates | coordination-policy | sending agent via `/handoff`; receiver acknowledges. | High-risk cross-domain transfer or explicit request for durable artifact. | Rule 16 is SHOULD; the 3-field lightweight handoff summary is the default handoff path. | Handoff preserves what was built, what's missing, acceptance criteria, and ledger entry when Medium/High. | Baseline exists; Sprint 3 wiring landed; adoption still unverified |
...
| `.codex/START.md` | Codex session-start shortcut that bootstraps the adapter and routes to the Claude-equivalent onboarding flow. | technical-director | adapter-policy | technical-director. | Codex onboarding guidance or startup bootstrap changes. | `AGENTS.md`, `codex-sdd`, and `using-sdd` win on behavior; this file is a launch shortcut, not a new source of truth. | `scripts/codex-preflight.*`; direct prompt smoke test in Codex. | Operational |
