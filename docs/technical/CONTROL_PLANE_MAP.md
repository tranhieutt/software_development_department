...
| 6 | Merge / deliver | `commit` / `changelog` | `producer` (coordinates), `devops-engineer` (pipeline) | Working tree clean. CI green. `CHANGELOG.md` updated. User approved commit + push. | CI red → return to `/diagnose`. Release gate failure → `release-manager`. | `CHANGELOG.md`, git history. Ledger entry for release cut. |
